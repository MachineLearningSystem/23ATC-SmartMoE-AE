...
def draw(fig, axs, nnodes, mbs, lat, layer_idx):
    ...
    with open(f'../logs/fig13/freq100', 'r') as f:
        line = f.readlines()[0]
    ...
    with open(f'../logs/fig13/changed_freq100', 'r') as f:
        lines = f.readlines()
    ...
    with open(f'../logs/fig13/freq10', 'r') as f:
        line = f.readlines()[0]
        data = json.loads(line)
        t0 = data['FastMoE'][1:]
        t1 = data['FasterMoE'][1:]
    ...
    with open(f'../logs/fig13/changed_freq10', 'r') as f:
        lines = f.readlines()
        x_data = [int(v) for v in lines]
        y_data = [t2[i//10] for i in x_data]
        x_data = np.array(x_data)
